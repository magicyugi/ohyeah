<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="NewVisit.aspx.cs"
    Inherits="AppBox.View.Visit.NewVisit" %>
    ...
    <script type="text/javascript">
        ...
        $().ready(function () {
            ...
            if (request("ProcessFlag") == "1") {
                $("#btnDeal").html("<span class='l-btn-left'><span class='l-btn-text icon-save l-btn-icon-left'>签订合同</span></span>");
                $("#btnChange").css("display", "none");
            }
            else if (request("ProcessFlag") == "2") {
                $("#btnDeal").html("<span class='l-btn-left'><span class='l-btn-text icon-save l-btn-icon-left'>修改合同</span></span>");
            }
            else if (request("ProcessFlag") == "3") {
                $("#btnDeal").html("<span class='l-btn-left'><span class='l-btn-text icon-save l-btn-icon-left'>二次销售</span></span>"); $("#btnDrop").css("display", "none");
                $("#btnChange").css("display", "none");
                //                $("#txtNextVisitDate").attr("required", "");
                //                $("#txtNextVisitContent").attr("required", "");
            }
            $.ajax({
                url: url,
                data: "type=dropdown",
            ...
            $("#btnSave").click(function () {
                ...
                if (setValidate($("#formVisit")) && request("ProcessFlag") != "3") {
                    $("#txtVisitType").val($("#txtVisitTypeCode").combobox("getText"));
                    SaveData(url, namecode, request("code"), "../Alerts/AlertList.aspx?ProcessFlag=" + request("ProcessFlag"));
                }
                else if (setValidate($("#formVisit"))) {
                    $("#txtVisitType").val($("#txtVisitTypeCode").combobox("getText"));
                    SaveData(url, namecode, request("code"), "../Alerts/AlertList.aspx?ProcessFlag=" + request("ProcessFlag"));
                } 
            });
            ...
            $("#btnSubmit").click(function () {
                if (setValidate($("#formReason"))) {
                    SaveData("Ajax.aspx", "Visit,reason", request("code") + "&reason=" + $("#txtReason").val(), "../Alerts/AlertList.aspx?ProcessFlag=" + request("ProcessFlag"));
                }
            })
            $("#btnDeal").click(function () {
                $("#hdndealinput").val("1");
                $("#btnSave").click();
                //window.location.href = "../Products/Shop.aspx?type=product&SaleBill_Code=" + request("SaleBill_Code") + "&processflag=" + request("ProcessFlag") + "&code=" + request("code") + "&id=" + request("id") + "&nextdate=" + $("#txtNextVisitDate").datebox("getValue") + "&visitcontent=" + $("#txtNextVisitContent").val() + "&content=" + $("#txtVisitContent").val();
                //ShowEditWin(namecode, "reason");
            });
            $("#btnChange").click(function () {
                if (confirm("是否确认转为售后客户!")) {
                    $("#hdndealinput").val("2");
                    $("#btnSave").click();
                }
...
<%-- <script type="text/javascript">
    ...
    <input type="hidden" name="hdndealinput" id="hdndealinput"   class="fieldItem" />
    <div data-options="region:'north',border:false" style="padding: 10px 0 0; height: 450px" 
            ...
            <div id="topLoader" style=" z-index:99; position:absolute"> </div>
                <ul>
                    ...
                    <li class="normalli">
                        <label for="txtMobile">
                            客户评级:</label> 
                          <%--  <input type="text" name="CustomerLevelName" id="txtCustomerLevel1" disabled="disabled" class="fieldItem"
                                field="CustomerLevelName" />--%>
                      <input type="combo"  id="CustomerLevel" name="CustomerLevel" class="easyui-combobox fieldItem"
                           field="CustomerLevel" /></li> 
                   <li class="tableli"> 
                       <label >
                            工作进度:</label> 
                    ...
                    <li class="normalli">
                        <label for="txtVisitTitle">
                            主题:</label>
                           <input type="text" name="VisitTitle" id="txtVisitTitle" class=" easyui-validatebox fieldItem" 
                                field="VisitTitle"  required="true" />
                          </li> 
                          <li class="comboli" style="display:none;">
                     ...
                     <li class="allli">
                        <label for="txtVisitContent" style="margin-right:10px;" >
                            内容:</label><textarea name="VisitContent" id="txtVisitContent" style="width:71.5%;"  rows="5" class="fieldItem" field="VisitContent"  ></textarea>
                     </li> 
                     <li class="normalli" >
                        <label for="txtNextVisitTime"   >
                           ...
                           <input type="checkbox" style="display:none;" id="txtMsgFlag" name="MsgFlag" class="fieldItem" title="是否短信提醒" field="MsgFlag" /> 
                     </li> 
                     <li class="normalli">
                    <%-- <div style="float:left">是否短信提醒</div>--%>
                        <label for="txtNextVisitContent"   >
                            联系计划:</label><input name="NextVisitContent" id="txtNextVisitContent"  class="easyui-validatebox fieldItem" field="NextVisitContent"    required="true"     /> 
                    </li>
        ...
        <div data-options="region:'south',border:false" style="text-align: center; padding: 10px 0 0;"
            ...
            <a class="easyui-linkbutton" id="btnDeal" data-options="iconCls:'icon-save'">签订合同</a>   
            <a class="easyui-linkbutton" id="btnChange" data-options="iconCls:'icon-save'">转为售后</a>   
    ...
    <div id="winProcess" class="easyui-window" data-options="title:'进度设置',modal:true,closed:true,iconCls:'icon-save',collapsible:false,minimizable:false"
        ...
        <form name="formReason" id="form1" status="reason">
            ...
            data-options=" 
                ...
                method: 'get', 
                view:groupview,
                url: 'Ajax.aspx?type=PGrid&code=' + request('code'),
